...
<html lang="pt-BR">
    ...
    <div class="container">   
      ...
      <div class="row text-center">
        ...
        <?php
          ...
          $iddCrime = $_GET['idCrime'] ?? '';

          $sql = "SELECT * FROM tipo_crime WHERE idCrime = $iddCrime";
  ...
  <div class="container">
    <div class="row">   
      <div class="col-10">
        <h3 class="p-5 text-muted">Alterar Tipo de Crime</h3>
      </div>
      <div class="col-2 p-5">
        <a href="crud_crime.php" class="btn btn-dark">Retornar</a>
      </div>
    </div>
      <form class="p-3 mt-4" action="editar_crime_scripts.php" method="POST" >
      <div class="form-row">
        <div class="form-group col-md-6 p-3">
          <label class="form-label text-muted" for="desTipoCrime">Descrição/Nome:</label>
          <input type="text" class="form-control text-muted" name="desTipoCrime" required value="<?php echo $linha['desTipoCrime']; ?>">
        </div>
      </div>
        <div class="row">
          <div class="col-6 text-center mt-3">
            <button type="submit" class="btn btn-dark mx-3 mt-3">Alterar Crime</button>
            <input type="hidden" name="iddCrime" value="<?php echo $linha['idCrime']; ?>">
          </div>
        </div>
    </form>
  </div>
